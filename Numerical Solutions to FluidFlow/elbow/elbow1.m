...
for istep=1:MaxStep
    ...
    for i=1:nz
        vt(i,1)=2*q3(i,1)*(sf(i,1)-sf(i,2))/J(i,1)^2;
        vt(i,ne)=2*q3(i,ne)*(sf(i,ne)-sf(i,ne-1))/J(i,ne)^2;
    end;
    for j=2:ne-1
...
figure(1);
hold on;
subplot(211);contourf(x,y,vt);
title(sprintf('Vorticity:time= %5.3f (visc.= %5.3f dt = %5.3f),%dX%d grid',t,alpha,dt,nz,ne));
subplot(212);contourf(x,y,sf);
title(sprintf('Stream Function:time= %5.3f (visc.= %5.3f dt = %5.3f),%dX%d grid',t,alpha,dt,nz,ne));
hold off;
pause(0.1);
